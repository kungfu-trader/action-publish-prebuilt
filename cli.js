...
if (argv.artifactsPath && argv.bucketStaging) {
    ...
    lib.addPreviewComment(argv.token, argv.owner, argv.repo, argv.pullRequestNumber, argv.bucketStaging).catch(console.error);
}

if (argv.bucketStaging && argv.bucketRelease) {
    lib.publish(argv.repo, argv.bucketStaging, argv.bucketRelease);
    lib.clean(argv.repo, argv.bucketStaging);
    lib.deletePreviewComment(argv.token, argv.owner, argv.repo, argv.pullRequestNumber, argv.bucketStaging).catch(console.error);
}
